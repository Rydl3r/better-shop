...
const Product = ({product, fetchProducts, setIsPopup}) => {
    return ( 
        <div className="bg-gray-200 rounded py-5 max-w-xs mx-auto my-5 flex flex-col justify-center items-center">
            ...
            <div className="px-5 py-3">{product.description ? product.description.replace( /(<([^>]+)>)/ig, '') : ""}</div>
            <div className="pb-3">Number of items -<b>{product.quantity}</b></div>
            <button onClick={() => {
            ...
            }} className="transition-all border border-black py-3 px-8 rounded-full hover:bg-white">Remove from cart</button>
        </div>
    )
}
